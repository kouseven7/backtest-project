...
- ⚠️ **GC戦略、VWAP_Bounce戦略などの基本戦略名が記録されていない**
...
**問題点**:
- ✅ GC戦略、VWAP_Bounce戦略などの**基本戦略名が失われる**
...
# 期待（基本戦略名を記録）
'strategy_name': 'GCStrategy'  # または
'strategy_name': 'ContrarianStrategy'  # または
'strategy_name': 'VWAPBreakoutStrategy'  # など

# 注: 銘柄コード（8001等）は別フィールド'symbol'で管理される
```

**戦略名形式の確認証拠** (2025-12-14調査):

**証拠A**: `src/dssms/dssms_integrated_main.py` Lines 2605-2606
```python
strategies = [
...
**証拠B**: `main_system/execution_control/multi_strategy_manager_fixed.py` Lines 264-274
```python
strategy_classes = {
...
**確認結果**:
- ✅ DSSMS本体、multi_strategy_manager、main_newの3モジュールで統一された戦略名形式
- ✅ 戦略名には銘柄コード（8001等）は含まれない
- ✅ 銘柄コードは別フィールド`'symbol'`で管理される
...
**問題点**:
1. GC戦略、VWAP_Bounce戦略などの基本戦略名が完全に失われる
...
**目的**: 実際の基本戦略名（GCStrategy、ContrarianStrategy等）を記録する

**推奨実装**:
```python
def _get_active_strategy_name(self, symbol: str) -> str:
    """アクティブな基本戦略名を取得"""
    # main_new.pyの戦略選択結果から取得
    # 戦略名形式: 'GCStrategy', 'ContrarianStrategy', 'VWAPBreakoutStrategy'等
    
    # 実際の実装例:
    # - main_new.pyから受け取ったstrategy_selection['selected_strategies']を保存
    # - 現在のシンボルでアクティブな戦略名を返す
    
    # 注: 銘柄コード（8001等）は含めない（別フィールド'symbol'で管理）
    
    if hasattr(self, 'active_strategies') and symbol in self.active_strategies:
        return self.active_strategies[symbol]  # 'GCStrategy'等
...
- dssms_trades.csvのstrategyカラムに基本戦略名が記録される
  - 例: `'GCStrategy'`, `'ContrarianStrategy'`, `'VWAPBreakoutStrategy'`
- 銘柄コードは`symbol`カラムで管理（例: `'8306'`, `'6758'`）

**戦略名形式の根拠** (2025-12-14調査確認):
- DSSMS本体: Lines 2605-2606で定義された戦略リスト
- multi_strategy_manager: Lines 264-274で同一形式使用
- 3モジュール（DSSMS、multi_strategy_manager、main_new）で統一
