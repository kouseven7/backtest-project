...
class DSSMSScheduler:
    ...
    def _write_daily_snapshot(self, price_fetch_ok: bool = True, market_value: Optional[float] = None) -> None:
        ...
        try:
            ...
            unrealized_pnl = 0.0
            # market_value が呼び出し元から渡されない場合はフォールバックで自前計算
            _market_value_provided = market_value is not None
            if not _market_value_provided:
                market_value = 0.0
            ...
            if _market_value_provided:
                self.logger.info(f"[SNAPSHOT] market_value={market_value:,.0f}（呼び出し元から受け取り）")
            # --- R-6 計算ブロック終わり ---
    ...
    def run_morning_screening(self, force: bool = False) -> str:
        ...
        try:
            ...
            for sym, pos in self.positions.items():
                try:
                    ...
                    market_value_calc += cur_price * qty if cur_price and qty else 0
            ...
            summary_data = {
                ...
                'total_assets': self.paper_balance.balance + market_value_calc,
                'daily_pnl': 0,
        ...
        # market_value_calc を計算して渡す（_get_current_price の二重呼び出しを防ぐ）
        market_value_calc = 0.0
        for sym, pos in self.positions.items():
            try:
                cur_price = self._get_current_price(sym)
                qty = int(pos.get("quantity", 0) or 0)
                if cur_price and qty:
                    market_value_calc += cur_price * qty
            except Exception:
                pass
        self._write_daily_snapshot(price_fetch_ok=True, market_value=market_value_calc)
        ...
        try:
            ...
            for sym, pos in self.positions.items():
                try:
                    ...
                    pass
            self._write_daily_snapshot(price_fetch_ok=price_fetch_ok, market_value=market_value_calc)
            self._write_last_run(
                status="success" if not error_occurred else "error",
                price_fetch_ok=price_fetch_ok,
                positions_held=len(self.positions),
