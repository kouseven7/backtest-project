...
class DSSMSIntegratedBacktester:
    ...
    def _advanced_ranking_selection(self, filtered_symbols: List[str], target_date: datetime) -> str:
        ...
        if self.advanced_ranking_engine and len(filtered_symbols) > 0:
            try:
                ...
                selected = self._integrated_ranking_selection_optimized(
                    filtered_symbols, target_date
                )
                
                if selected:
                    self.logger.info(f"[RANK_SCORE] 銘柄選択結果: {selected} / 候補{len(filtered_symbols)}銘柄 @ {target_date.strftime('%Y-%m-%d')}")
                    self.logger.info(
                        f"{len(filtered_symbols)}銘柄最適選択)"
                    )
                    return selected
                
                selected = self._legacy_advanced_ranking_selection(filtered_symbols, target_date)
                self.logger.info(f"[RANK_SCORE] 銘柄選択結果: {selected} / 候補{len(filtered_symbols)}銘柄 @ {target_date.strftime('%Y-%m-%d')}")
                return selected
    ...
    def _get_optimal_symbol(self, target_date: datetime, 
        ...
        try:
            start_time = time.time()
            print(f"[RANK_DEBUG] _get_optimal_symbol called: date={target_date.strftime('%Y-%m-%d')}, dss_core={self.dss_core is not None}, screener={self.nikkei225_screener is not None}")
            ...
            if self.dss_core and dss_available:
                ...
                if selected_symbol:
                    ...
                    print(f"[RANK_DEBUG] dss_core経路: selected={selected_symbol}")
            ...
            if self.nikkei225_screener:
                try:
                    ...
                    if filtered_symbols:
                        print(f"[RANK_DEBUG] screener経路: filtered={len(filtered_symbols) if filtered_symbols else 0}銘柄")
                        ...
                        print(f"[RANK_DEBUG] 最終return: {selected}")
                        return selected
                except Exception as e:
                    # Error handling
                    pass
    ...
    def _execute_multi_strategies_daily(self, target_date: datetime, symbol: str, stock_data, force_close_position: dict = None) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                # Sprint 3C: market_regimeをbacktest_dailyに渡す（戦略側フィルタ用）
                if market_analysis and 'market_regime' in market_analysis:
                    kwargs['market_regime'] = market_analysis['market_regime']
            ...
            if result['action'] in ['buy', 'sell'] and result['signal'] != 0:
                ...
                if result['action'] == 'buy':
                    # [SHARES_ZERO_GUARD] shares=0チェック（Task-A: _calculate_lot_sharesが高額銘柄で0を返す場合の防御）
                    if result.get('shares', 0) == 0:
                        ...
                        # LOT_GUARD: 単元未満株の発注スキップ（バックテスト経路）
                        if result['shares'] == 0:
                            self.logger.warning(
                                f"[LOT_GUARD] 単元未満のためエントリースキップ: "
                                f"{symbol}, 株価={result['price']:.0f}円, shares=0"
                            )
                        else:
