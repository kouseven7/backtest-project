...
def test_fetch_strategy_required_data():
    ...
    print("\n[TOOL] fetch_strategy_required_data関数テスト")
    
    stock_data = create_sample_stock_data()
    
    try:
        # VWAPBreakoutStrategy用データ取得テスト
        print("[LIST] VWAPBreakoutStrategy用データ取得...")
        ...
        if 'index_data' in vwap_data and vwap_data['index_data'] is not None:
            print(f"[OK] VWAPBreakoutStrategy用index_data取得成功: {len(vwap_data['index_data'])} rows")
            print(f"   Columns: {list(vwap_data['index_data'].columns)}")
        else:
            print("[ERROR] VWAPBreakoutStrategy用index_data取得失敗")
            return False
        
        # OpeningGapStrategy用データ取得テスト
        print("[LIST] OpeningGapStrategy用データ取得...")
        ...
        if 'dow_data' in gap_data and gap_data['dow_data'] is not None:
            print(f"[OK] OpeningGapStrategy用dow_data取得成功: {len(gap_data['dow_data'])} rows")
            print(f"   Columns: {list(gap_data['dow_data'].columns)}")
        else:
            print("[ERROR] OpeningGapStrategy用dow_data取得失敗")
            return False
            
        return True
        
    except Exception as e:
        print(f"[ERROR] fetch_strategy_required_dataテストエラー: {str(e)}")
        return False

def test_multi_strategy_manager_initialization():
    """MultiStrategyManagerの初期化テスト"""
    print("\n[TOOL] MultiStrategyManager初期化テスト")
    
    try:
        manager = MultiStrategyManager()
        print("[OK] MultiStrategyManager基本初期化成功")
        
        # システム初期化の実行
        print("[LIST] システム初期化実行...")
        ...
        if init_success:
            ...
            if hasattr(manager, 'strategy_registry') and manager.strategy_registry:
                print(f"[OK] Strategy registry確認: {len(manager.strategy_registry)} strategies")
                return True
            else:
                print("[WARNING]  Strategy registryは空ですが、初期化は成功")
                return True  # 空でも初期化成功とみなす
        else:
            print("[ERROR] システム初期化失敗")
            return False
            
    except Exception as e:
        print(f"[ERROR] MultiStrategyManager初期化エラー: {str(e)}")
        return False
...
def main():
    ...
    if overall_success:
        print("[SUCCESS] Phase 3簡易統合テスト成功 - 基本機能正常動作")
        print("[LIST] 次ステップ: より詳細な統合テストまたはPhase 4実装")
    else:
        print("[WARNING]  Phase 3簡易統合テスト失敗 - 基本機能に問題あり")
