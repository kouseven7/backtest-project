...
try:
    ...
    print(f'✅ TODO #8エラー戦略復旧: {len(recovered_strategies)}/{len(target_strategies)} - {recovered_strategies}')
    
    # 戦略レジストリステータス
    registry_status = manager.get_registry_status()
    print(f'✅ レジストリステータス: 初期化={registry_status["is_initialized"]}, 戦略数={registry_status["total_strategies"]}')
    
    # 戦略インスタンス化テスト（可能な場合）
    if available_strategies:
        test_strategy = available_strategies[0]
        print(f'\n🧪 戦略インスタンス化テスト: {test_strategy}')
        ...
        try:
            ...
            print(f'✅ インスタンス化成功: {test_strategy}, backtest()={has_backtest}')
        except Exception as e:
            print(f'⚠️ インスタンス化テスト: {test_strategy} - {e}')
    
    print(f'\n🎯 TODO #9 成功指標確認:')
    print(f'  戦略レジストリ実装: {"✅" if len(available_strategies) > 0 else "❌"}')
    print(f'  TODO #8エラー戦略復旧: {"✅" if len(recovered_strategies) >= 2 else "❌"}')
    print(f'  バックテスト基本理念準拠: {"✅" if init_success else "❌"}')
    
    if len(recovered_strategies) >= 2 and init_success:
        print('🎉 TODO #9: 戦略レジストリシステム完全実装 - 成功！')
    else:
        print('⚠️ TODO #9: 部分的成功 - 追加調整が必要')
