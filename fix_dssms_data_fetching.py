...
def test_yfinance_availability():
    ...
    try:
        ...
        print(f"📊 テストデータ取得: {test_symbol} ({start_date.date()} → {end_date.date()})")
...
def create_real_data_backtest():
    ...
    try:
        ...
        print(f"📊 短期テストバックテスト: {start_date.date()} → {end_date.date()}")
