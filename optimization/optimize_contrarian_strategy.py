"""
Contrarian戦略（逆張り戦略）の最適化を実行するモジュール
"""
import sys
import os
import time
...
def optimize_contrarian_strategy(data, use_parallel=False):
    ...
    Returns:
        pd.DataFrame: 最適化結果
    """
    logger.info("Contrarian戦略の最適化を開始します。")
    ...
    # 出力ディレクトリの設定
    output_dir = os.path.join("backtest_results", "optimization", "contrarian_strategy")
...
def main():
    """メイン関数"""
    parser = argparse.ArgumentParser(description='Contrarian戦略の最適化')
    ...
    try:
        ...
        # Contrarian戦略の最適化を実行
        results = optimize_contrarian_strategy(stock_data, use_parallel=args.parallel)
