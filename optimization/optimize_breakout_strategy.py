...
def optimize_breakout_strategy(data, use_parallel=False):
    ...
    Returns:
        pd.DataFrame: 最適化結果
    """
    logger.info("Breakout戦略の最適化を開始します。")
    ...
    # 出力ディレクトリの設定
    output_dir = os.path.join("backtest_results", "optimization", "breakout_strategy")
    ...
    # 結果の保存
    filename = f"breakout_strategy_results_{timestamp}"
...
def main():
    """メイン関数"""
    parser = argparse.ArgumentParser(description='Breakout戦略の最適化')
    ...
    try:
        ...
        # Breakout戦略の最適化を実行
        results = optimize_breakout_strategy(stock_data, use_parallel=args.parallel)
