...
def diagnose_switch_count_mechanism():
    ...
    try:
        ...
        print("✅ DSSMSBacktesterのインポート成功")
        
        # 2. バックテスター初期化とswitch_history確認
        backtester = DSSMSBacktester()
        
        print(f"📊 初期化後のswitch_history長: {len(backtester.switch_history)}")
        print(f"📊 初期化後のperformance_history長: {len(backtester.performance_history)}")
        ...
        print(f"\n🧪 テストシミュレーション実行: {start_date.date()} - {end_date.date()}")
        ...
        print(f"✅ シミュレーション完了")
        print(f"📊 実行後のswitch_history長: {len(backtester.switch_history)}")
        ...
        if hasattr(backtester, '_evaluate_switch_decision'):
            print("  ✅ _evaluate_switch_decision メソッド存在")
        else:
            print("  ❌ _evaluate_switch_decision メソッド未実装")
        
        # intelligent_switch_managerの存在確認
        if hasattr(backtester, 'intelligent_switch_manager'):
            print(f"  ✅ intelligent_switch_manager 存在: {type(backtester.intelligent_switch_manager)}")
        else:
            print("  ❌ intelligent_switch_manager 未設定")
        
        # hierarchical_ranking_systemの存在確認
        if hasattr(backtester, 'ranking_system'):
            print(f"  ✅ ranking_system 存在: {type(backtester.ranking_system)}")
        else:
            print("  ❌ ranking_system 未設定")
...
def diagnose_data_persistence():
    ...
    try:
        ...
        print(f"❌ データ永続化調査エラー: {e}")
        traceback.print_exc()
        return {'success': False, 'error': str(e)}

def diagnose_engine_integration():
    """統一エンジン統合状況の確認"""
    print(f"\n🔍 Task 1.3: 統一エンジン統合状況確認")
    print("-" * 40)
    ...
    for engine_file in engine_files:
        ...
        if engine_path.exists():
            print(f"✅ {engine_file} 存在 ({engine_path.stat().st_size} bytes)")
            engine_status[engine_file] = {'exists': True, 'size': engine_path.stat().st_size}
        else:
            print(f"❌ {engine_file} 不存在")
    ...
    try:
        ...
        print(f"✅ DSSMSExcelExporterV2 インポート成功")
        engine_status['DSSMSExcelExporterV2'] = {'available': True}
    except Exception as e:
        print(f"❌ DSSMSExcelExporterV2 インポートエラー: {e}")
...
def main():
    ...
    if results['switch_count']['success']:
        ...
        print(f"✅ 切替機構診断: 成功 (検出された切替数: {switch_count})")
        
        if switch_count < 10:
            print("  ⚠️ 切替数が異常に少ない - 切替判定ロジック調査が必要")
        else:
            print("  ✅ 切替数は正常範囲")
    else:
        print(f"❌ 切替機構診断: 失敗")
