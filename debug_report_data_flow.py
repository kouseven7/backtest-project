...
def debug_main_data_structure():
    """main.pyが生成するデータ構造を調査"""
    print("🔍 main.pyデータ構造調査開始")
    
    # 最新のCSVファイルを読み込み（main.pyの出力）
    csv_files = []
    for root, dirs, files in os.walk("output"):
        for file in files:
            if file.endswith('.csv') and 'backtest' in file.lower():
                csv_files.append(os.path.join(root, file))
    
    if not csv_files:
        print("❌ バックテストCSVファイルが見つかりません")
        return None
...
def debug_signal_columns(stock_data):
    """シグナル列の詳細調査"""
    print(f"\n🎯 シグナル列調査:")
...
def debug_trade_extraction(stock_data):
    """取引抽出プロセスの詳細調査"""
    print(f"\n⚙️ 取引抽出プロセス調査:")
...
def debug_final_price_issue(stock_data):
    """エグジット価格固定問題の詳細調査"""
    print(f"\n🔍 エグジット価格固定問題調査:")
...
def main():
    print("🚀 レポートデータ流れデバッグ開始")
    print("="*50)
    
    # ステップ1: データ構造調査
    stock_data = debug_main_data_structure()
    if stock_data is None:
        return
    
    # ステップ2: シグナル列調査
    debug_signal_columns(stock_data)
    
    # ステップ3: 取引抽出調査
    trades = debug_trade_extraction(stock_data)
    
    # ステップ4: エグジット価格問題調査
    debug_final_price_issue(stock_data)
