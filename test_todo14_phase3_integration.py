...
def test_phase3_auto_supply_vwap():
    ...
    print("\n[TOOL] Phase 3テスト: VWAPBreakoutStrategy自動データ供給")
    ...
    try:
        ...
        print("[LIST] テスト: index_data未提供時の自動供給...")
        ...
        if hasattr(strategy_instance, 'index_data'):
            print(f"[OK] 自動供給成功: index_data有り ({len(strategy_instance.index_data)} rows)")
            print(f"   Index data columns: {list(strategy_instance.index_data.columns)}")
            return True
        else:
            print("[ERROR] 自動供給失敗: index_dataが設定されていません")
            return False
            
    except Exception as e:
        print(f"[ERROR] Phase 3自動供給エラー: {str(e)}")
        return False

def test_phase3_auto_supply_opening_gap():
    """Phase 3: OpeningGapStrategy 自動データ供給テスト"""
    print("\n[TOOL] Phase 3テスト: OpeningGapStrategy自動データ供給")
    ...
    try:
        ...
        print("[LIST] テスト: dow_data未提供時の自動供給...")
        strategy_instance = manager.get_strategy_instance(
        ...
        if hasattr(strategy_instance, 'dow_data'):
            print(f"[OK] 自動供給成功: dow_data有り ({len(strategy_instance.dow_data)} rows)")
            print(f"   Dow data columns: {list(strategy_instance.dow_data.columns)}")
            return True
        else:
            print("[ERROR] 自動供給失敗: dow_dataが設定されていません")
            return False
...
def test_manual_data_provision():
    ...
    try:
        print("[LIST] テスト: 手動でindex_data提供...")
        strategy_instance = manager.get_strategy_instance(
            strategy_name='VWAPBreakoutStrategy',
            data=stock_data,
            params={'vwap_window': 20, 'breakout_threshold': 0.02},
        ...
        if hasattr(strategy_instance, 'index_data'):
            print(f"[OK] 手動データ提供成功: index_data有り ({len(strategy_instance.index_data)} rows)")
            return True
        else:
            print("[ERROR] 手動データ提供失敗")
            return False
            
    except Exception as e:
        print(f"[ERROR] 手動データ提供エラー: {str(e)}")
        return False

def test_real_data_fetcher_direct():
    """RealMarketDataFetcher直接テスト（Phase 2確認）"""
    print("\n[TOOL] RealMarketDataFetcher直接テスト")
...
def main():
    ...
    print(f"\n[CHART] 総合結果: {success_count}/{len(results)} tests passed")
    
    if overall_success:
        print("[SUCCESS] Phase 3統合テスト成功 - 自動データ供給システム正常動作")
        print("[LIST] 次ステップ: Phase 4 (MarketDataQualityValidator) 実装準備完了")
    else:
        print("[WARNING]  Phase 3統合テスト部分的失敗 - 問題箇所の調査が必要")
