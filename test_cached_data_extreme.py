#!/usr/bin/env python3
"""
キャッシュデータを使った極限緩和パラメータでのVWAPBounceStrategy動作テスト
"""
...
def test_with_cached_data():
    """キャッシュデータを使った極限緩和パラメータでの動作テスト"""
    print("=== キャッシュデータを使った極限緩和パラメータテスト ===")
...
def test_multiple_relaxed_params():
    """複数の段階的緩和パラメータでのテスト"""
    print("\n=== 段階的緩和パラメータテスト ===")
    ...
    for param_set in param_sets:
        ...
        try:
            strategy = VWAPBounceStrategy(data.copy(), params)
            results = strategy.backtest()
