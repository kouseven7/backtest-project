...
def analyze_performance_results():
    ...
    logger.info(f"📊 結果ファイル: {latest_file.name}")
    ...
    try:
        ...
        if solved_100_loss:
            logger.info(f"   ✅ -100%損失問題: 解決済み ({total_return:.2f}%)")
        else:
            logger.error(f"   ❌ -100%損失問題: 未解決 ({total_return:.2f}%)")
        
        if has_trades:
            logger.info(f"   ✅ 取引生成: 正常 ({metrics.get('総取引数', 0)}件)")
        else:
            logger.error(f"   ❌ 取引生成: 失敗")
        
        if profit_factor > 0:
            logger.info(f"   ✅ プロフィットファクター: 正常 ({profit_factor:.2f})")
        else:
            logger.error(f"   ❌ プロフィットファクター: 異常 ({profit_factor:.2f})")
        ...
        if total_return > 0:
            logger.info(f"   🎉 プラスリターン達成: {total_return:.2f}%")
        elif total_return > -10:
            logger.warning(f"   ⚠️  軽微な損失: {total_return:.2f}% (許容範囲)")
        else:
            logger.error(f"   ❌ 大幅な損失: {total_return:.2f}%")
        
        if win_rate > 50:
            logger.info(f"   ✅ 良好な勝率: {win_rate:.1f}%")
        else:
            logger.warning(f"   ⚠️  勝率改善の余地: {win_rate:.1f}%")
...
def validate_signal_generation():
    ...
    try:
        ...
        if not result.empty:
            ...
            logger.info(f"📊 Perfect Order検出結果:")
            ...
            if entry_signals > 50:
                logger.info("   ✅ 豊富なシグナル生成")
            elif entry_signals > 20:
                logger.info("   ⚠️  適度なシグナル生成")
            else:
                logger.warning("   📉 シグナル生成が少ない")
...
def main():
    ...
    if performance_result and performance_result['problem_solved']:
        logger.info("🎉 DSSMS -100%問題解決成功！")
        logger.info("✅ Perfect Order検出器の修正が有効でした")
        logger.info("📈 システムが正常に取引を実行しています")
        
        if performance_result['total_return'] > 0:
            logger.info("🏆 プラスリターンも達成")
        
        if signal_ok:
            logger.info("🔧 シグナル生成システム正常動作")
        ...
        logger.warning("⚠️  部分的改善は見られますが、更なる調整が必要")
        logger.info(f"💡 現在のリターン: {performance_result['total_return']:.2f}%")
        
    else:
        logger.error("❌ 結果確認に失敗しました")
