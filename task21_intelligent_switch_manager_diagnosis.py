...
def diagnose_intelligent_switch_manager():
    ...
    print("🔍 Task 2.1: IntelligentSwitchManager統合問題診断開始")
    print("="*60)
    ...
    try:
        ...
        if hasattr(backtester, 'switch_manager'):
            ...
            print(f"📊 switch_manager属性存在: {switch_manager is not None}")
            
            if switch_manager is not None:
                print(f"📊 switch_manager型: {type(switch_manager)}")
                print(f"📊 switch_managerメソッド数: {len([m for m in dir(switch_manager) if not m.startswith('_')])}")
        ...
        if hasattr(backtester, '_evaluate_switch_decision'):
            ...
            try:
                ...
                print(f"⚠️ ソースコード取得失敗: {e}")
        else:
            print("❌ _evaluate_switch_decision メソッドなし")
        ...
        for constraint, current_value in constraints.items():
            if hasattr(backtester, constraint):
                ...
                if constraint == 'min_holding_period_hours' and actual_value >= 24:
                    print("    ⚠️ 24時間制約が切替を抑制している可能性")
                    print("    📋 推奨: 12-18時間への短縮を検討")
...
if __name__ == "__main__":
    print("🚀 DSSMS切替数激減問題 - Task 2.1診断開始")
    print("="*80)
