"""
GCStrategy Metadata Regeneration Script - v2.0形式への変換

既存のv1.0形式メタデータをv2.0形式に変換し、create_strategy_metadata()で再生成します。
これにより、スコア計算時のネスト構造問題を根本的に解決します。

主な機能:
- 既存GCStrategyメタデータ(v1.0)からデータ抽出
...
def convert_v1_to_v2_trend_performance():
    """
    GCStrategy v1.0 trend_adaptabilityをv2.0形式に変換
    ...
    # v1.0からの既存データ（GCStrategy_characteristics.json）
    trend_performance = {
        "uptrend": {
            # performance_score 9.2 → suitability_score 0.92 (0-1スケール)
            "suitability_score": 0.92,
            
            # parameter_optimization_historyから抽出: sharpe_ratio 1.62
            "sharpe_ratio": 1.62,
            
            # 既存値
            "max_drawdown": 0.09,
            
            # performance_metricsから推定: win_rate 0.65
            "win_rate": 0.65,
            
            # 推定値: expectancy（上昇トレンドで高性能のため正の値）
            "expectancy": 0.025,
            
            # expected_return 0.12 → avg_return
            "avg_return": 0.12,
            
            # 既存値
            "volatility": 0.13,
            
            # parameter_optimization_historyから算出: total_return 0.15 / max_drawdown 0.09 = 1.67
            "calmar_ratio": 1.67,
        ...
        "downtrend": {
            # performance_score 2.8 → suitability_score 0.28
            "suitability_score": 0.28,
            ...
            # expected_return -0.05 → avg_return
            "avg_return": -0.05,
        ...
        "range-bound": {
            ...
            # expected_return 0.04 → avg_return
            "avg_return": 0.04,
...
def convert_v1_to_v2_volatility_performance():
    """
    GCStrategy v1.0 volatility_adaptabilityをv2.0形式に変換
    ...
    volatility_performance = {
        "high_volatility": {
            ...
            # 推定値: 高ボラティリティでは低めのsharpe
            "sharpe_ratio": 0.65,
            
            # 推定値: 高ボラティリティでは大きいdrawdown
            "max_drawdown": 0.18,
        ...
        "medium_volatility": {
            # performance_score 8.5 → suitability_score 0.85
        ...
        "low_volatility": {
            # performance_score 7.2 → suitability_score 0.72
            "suitability_score": 0.72,
            
            # 推定値: 低ボラティリティで中程度のsharpe
            "sharpe_ratio": 1.25,
...
def main():
    """GCStrategyメタデータをv2.0形式で再生成"""
    
    print("=" * 80)
    print("GCStrategy Metadata Regeneration - v2.0形式への変換")
    print("=" * 80)
    ...
    existing_file = os.path.join(
        ...
        "GCStrategy_characteristics.json"
    )
    ...
    print("\n" + "=" * 80)
    print("GCStrategyメタデータの再生成が完了しました")
    print("=" * 80)
    print(f"\n次の作業:")
    print(f"1. main_new.pyを実行してバックテスト検証")
    print(f"2. GCStrategyのスコア計算成功を確認")
