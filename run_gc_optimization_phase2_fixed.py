"""
GC戦略 Phase 2最適化スクリプト（修正版）- stop_loss最適化 + 動的トレーリングストップ実装
...
- short_window, long_window, exit_on_death_cross等の重要なパラメータを削除しない

主な機能:
- update_gc_params_partial(): 部分的なパラメータ更新（既存構造を保持）
...
セーフティ機能/注意事項:
- バックアップ自動作成: gc_strategy_signal_backup_phase2_fixed_YYYYMMDD.py
...
# 出力先ディレクトリ
OUTPUT_DIR = PROJECT_ROOT / "output" / "gc_optimization_phase2_fixed"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# gc_strategy_signal.pyのパス
GC_STRATEGY_FILE = PROJECT_ROOT / "strategies" / "gc_strategy_signal.py"
GC_STRATEGY_BACKUP = PROJECT_ROOT / "strategies" / f"gc_strategy_signal_backup_phase2_fixed_{datetime.now().strftime('%Y%m%d')}.py"
...
def update_gc_params_partial(take_profit: float, trailing_stop: float, stop_loss: float):
    """
    部分的なパラメータ更新（既存構造を保持）
    
    既存のdefault_params内の特定キーのみを更新し、他のパラメータ（short_window, long_window等）を保持
    
    Args:
        take_profit: 利確閾値
        trailing_stop: トレーリングストップ閾値
        stop_loss: ストップロス閾値
    """
    with open(GC_STRATEGY_FILE, "r", encoding="utf-8") as f:
        content = f.read()
    
    # 各パラメータを個別に更新（正規表現で行単位で置換）
    # take_profit更新
    content = re.sub(
        r'"take_profit":\s*[0-9.]+',
    ...
    # trailing_stop_pct更新
    content = re.sub(
        r'"trailing_stop_pct":\s*[0-9.]+',
        f'"trailing_stop_pct": {trailing_stop}',
        content
    )
    ...
    print(f"パラメータ更新: take_profit={take_profit}, trailing_stop={trailing_stop}, stop_loss={stop_loss}")
...
def update_gc_params_phase2_1(take_profit: float, trailing_stop: float, stop_loss: float):
    ...
    # 部分的なパラメータ更新
    update_gc_params_partial(take_profit, trailing_stop, stop_loss)
...
def update_gc_params_phase2_2(take_profit: float, trailing_stop: float, stop_loss: float, 
    ...
    # 動的トレーリングストップ用の閾値パラメータを追加
    with open(GC_STRATEGY_FILE, "r", encoding="utf-8") as f:
        content = f.read()
    
    # default_params内にdynamic_trailing_threshold1/2が存在しない場合は追加
    if "dynamic_trailing_threshold1" not in content:
        # default_params の閉じ括弧の直前に追加
        content = re.sub(
            r'(\s+)"max_hold_days":\s*\d+,',
            rf'\1"max_hold_days": 300,\n\1"dynamic_trailing_threshold1": {threshold1},\n\1"dynamic_trailing_threshold2": {threshold2},',
            content
        )
    else:
        # 既存の閾値パラメータを更新
        content = re.sub(
            r'"dynamic_trailing_threshold1":\s*[0-9.]+',
            f'"dynamic_trailing_threshold1": {threshold1}',
            content
        )
        content = re.sub(
            r'"dynamic_trailing_threshold2":\s*[0-9.]+',
            f'"dynamic_trailing_threshold2": {threshold2}',
            content
        )
    
    # 動的トレーリングストップメソッド追加（存在しない場合）
    if "_get_dynamic_trailing_stop" not in content:
        dynamic_method = f'''
...
'''
    ...
    if match:
        ...
        # 動的トレーリングストップ呼び出しが存在しない場合は追加
        if "self._get_dynamic_trailing_stop" not in generate_exit_signal_content:
            ...
            generate_exit_signal_content = re.sub(
                ...
                r'\1        trailing_stop_pct = self._get_dynamic_trailing_stop(entry_price, current_price)\n',
    ...
    print(f"Phase 2-2パラメータ更新完了: threshold1={threshold1}, threshold2={threshold2}")
...
def save_summary(results: list):
    ...
    csv_path = OUTPUT_DIR / "optimization_summary_phase2_fixed.csv"
    df.to_csv(csv_path, index=False, encoding="utf-8-sig")
    print(f"比較表保存: {csv_path}")
    
    # JSON形式（詳細情報）
    json_path = OUTPUT_DIR / "all_results_phase2_fixed.json"
...
def main():
    print("=" * 80)
    print("GC戦略 Phase 2最適化スクリプト（修正版）開始")
    print(f"実行日時: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 80)
    print()
    
    # ログファイル作成
    log_file = OUTPUT_DIR / "execution_log_phase2_fixed.txt"
    with open(log_file, "w", encoding="utf-8") as f:
        f.write(f"GC戦略 Phase 2最適化スクリプト（修正版）実行ログ\n")
    ...
    if best_params:
        best_params_file = OUTPUT_DIR / "best_params_phase2_fixed.json"
        with open(best_params_file, "w", encoding="utf-8") as f:
            json.dump(best_params, f, indent=2, ensure_ascii=False)
        print(f"最適パラメータ保存: {best_params_file}")
        print()
        print("最適パラメータ（Phase 2修正版）:")
