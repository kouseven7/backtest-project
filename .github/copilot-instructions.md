...
### **テスト作成場所の厳格ルール（最優先・例外なし）**
- 新規テスト用Pythonファイルは必ず`tests/temp/`配下に作成する
- ルート直下（プロジェクトトップ）に`.py`ファイルを新規作成してはならない
- このルールは強制ルールとし、他の利便性より優先して厳格に遵守する
...
### **厳格運用ルール（必須）**
- テストコード作成時は、まず`tests/temp/`に作成すること
- ルート直下への`.py`ファイル新規作成は禁止
- 一時検証用途でも、例外なくこのルールを適用する
