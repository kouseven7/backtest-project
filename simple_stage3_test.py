...
def test_cache_integration():
    ...
    try:
        ...
        print(f"💰 時価総額データ: {market_cap_1 is not None}")
        ...
        print(f"💰 時価総額データ: {market_cap_2 is not None}")
        
        # キャッシュ効果
        if time_1 > 0 and time_2 < time_1:
            speedup = ((time_1 - time_2) / time_1) * 100
            print(f"🚀 キャッシュ高速化: {speedup:.1f}%")
        
        return True
        
    except Exception as e:
        print(f"❌ キャッシュ統合テスト失敗: {e}")
        return False
...
def test_algorithm_integration():
    ...
    try:
        ...
        print("✅ OptimizedAlgorithmEngine統合作成成功")
        ...
        print(f"❌ アルゴリズム統合テスト失敗: {e}")
        return False

def main():
    """メイン実行"""
    print("🚀 Stage 3統合 基本動作テスト開始")
    print("=" * 60)
    ...
    # 結果まとめ
    print("\n" + "=" * 60)
    print("📊 Stage 3統合テスト結果")
    print("=" * 60)
    print(f"SmartCache統合: {'✅ 成功' if cache_success else '❌ 失敗'}")
    print(f"AlgorithmEngine統合: {'✅ 成功' if algorithm_success else '❌ 失敗'}")
    
    overall_success = cache_success and algorithm_success
    print(f"\n🏆 総合結果: {'✅ Stage 3統合成功' if overall_success else '❌ Stage 3統合失敗'}")
