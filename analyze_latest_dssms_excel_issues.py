...
def analyze_latest_dssms_excel():
    ...
    try:
        ...
        if "戦略統計" in workbook.sheetnames:
            ...
            print(f"   [CHART] 戦略統計シート行数: {len(strategy_data)}")
            if strategy_data:
                print("   📝 先頭5行:")
            ...
            if len(strategy_data) > 1:  # ヘッダー行を除く
                ...
                print(f"   [UP] ユニーク戦略数: {len(unique_strategies)}")
                ...
                if missing_strategies:
                    print(f"   [WARNING]  不足戦略: {list(missing_strategies)}")
                
                # DSSMSのみの問題をチェック
                if unique_strategies == {"DSSMSStrategy"}:
                    print("   [ERROR] 問題確認: 戦略名が「DSSMSStrategy」のみになっています")
                elif len(unique_strategies) == 7:
                    print("   [OK] 戦略名は正常（7つの個別戦略）")
        ...
        if "取引履歴" in workbook.sheetnames:
            ...
            print(f"   [CHART] 取引履歴シート行数: {len(trade_data)}")
                ...
                if trade_data:
                    ...
                    if holding_period_index is not None:
                        ...
                        if len(unique_periods) == 1 and "24.0" in unique_periods:
                            print("   [ERROR] 問題確認: 保有期間が24時間固定になっています")
                        elif len(unique_periods) > 1:
                            print("   [OK] 保有期間は正常（多様な値）")
                        else:
                            print(f"   [WARNING]  保有期間の状況: {unique_periods}")
        ...
        if json_path.exists():
            ...
            print(f"   [ERROR] 対応JSONファイルが見つかりません: {json_path}")
