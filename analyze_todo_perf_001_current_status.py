...
def analyze_dssms_performance():
    ...
    if error:
        print(f"   [ERROR] インポートエラー: {error}")
        results['dssms_import'] = {'status': 'error', 'error': error}
    else:
        print(f"   [OK] インポート時間: {import_time:.1f}ms")
    ...
    try:
        ...
        if lazy_loader_path.exists():
            print(f"   [OK] lazy_loader存在: {lazy_loader_path}")
    ...
    try:
        ...
        if ultra_light_path.exists():
            ...
            if ul_error:
                print(f"   [ERROR] UltraLight版エラー: {ul_error}")
                results['ultra_light'] = {'status': 'error', 'error': ul_error}
            else:
                print(f"   [OK] UltraLight版インポート: {ul_time:.1f}ms")
                results['ultra_light'] = {'status': 'success', 'time_ms': ul_time}
        else:
            print("   [ERROR] UltraLight版未発見")
    ...
    for lib_name, import_name in heavy_libraries:
        ...
        if lib_error:
            print(f"   [ERROR] {lib_name}: エラー {lib_error}")
            library_times[lib_name] = {'status': 'error', 'error': lib_error}
        else:
            print(f"   [CHART] {lib_name}: {lib_time:.1f}ms")
    ...
    try:
        ...
        if results['dssms_import']['status'] == 'success':
            ...
            print(f"   [OK] 初期化時間: {init_time:.1f}ms")
            
            # 合計時間
            total_time = (init_end - start_time) * 1000
            print(f"   [CHART] 合計時間（インポート+初期化）: {total_time:.1f}ms")
            ...
            print("   [WARNING]  DSSMSインポートエラーのため実行テスト不可")
...
def analyze_perf_001_status(results):
    ...
    if results.get('lazy_loader_integration') == 'fully_removed':
        ...
        print("   [OK] Phase 2 lazy_loader問題: 解決済み")
        ...
        print("   [WARNING]  Phase 2 lazy_loader問題: 部分解決")
    else:
        print("   [ERROR] Phase 2 lazy_loader問題: 不明")
    ...
    if dssms_time < 100:  # 100ms以下なら良好
        ...
        print(f"   [OK] Phase 2 SymbolSwitchManager問題: 解決済み ({dssms_time:.1f}ms)")
        ...
        print(f"   [WARNING]  Phase 2 SymbolSwitchManager問題: 改善済みだが不十分 ({dssms_time:.1f}ms)")
    else:
        ...
        print(f"   [ERROR] Phase 2 SymbolSwitchManager問題: 未解決 ({dssms_time:.1f}ms)")
    ...
    if yfinance_time > 500 or openpyxl_time > 200:
        ...
        print(f"   [ERROR] Phase 3重いライブラリ: 未解決 (yfinance:{yfinance_time:.0f}ms, openpyxl:{openpyxl_time:.0f}ms)")
    else:
        evaluation['phase3_heavy_libraries'] = 'acceptable' 
        print(f"   [OK] Phase 3重いライブラリ: 許容範囲 (yfinance:{yfinance_time:.0f}ms, openpyxl:{openpyxl_time:.0f}ms)")
    ...
    if total_time < 1500:  # 目標1500ms
        ...
        print(f"   [OK] 全体パフォーマンス: 目標達成 ({total_time:.1f}ms < 1500ms)")
        ...
        print(f"   [WARNING]  全体パフォーマンス: 改善済みだが目標未達成 ({total_time:.1f}ms)")
    else:
        evaluation['overall_performance'] = 'poor'
        evaluation['remaining_issues'].append(f'深刻なパフォーマンス問題 ({total_time:.1f}ms)')
        print(f"   [ERROR] 全体パフォーマンス: 深刻な問題 ({total_time:.1f}ms)")
    
    # 残存課題数評価
    remaining_count = len(evaluation['remaining_issues'])
    if remaining_count == 0:
        print(f"\n   [TARGET] TODO-PERF-001総合評価: [OK] 完全解決")
    elif remaining_count <= 2:
        print(f"\n   [TARGET] TODO-PERF-001総合評価: [WARNING]  部分解決 ({remaining_count}件残存)")
    else:
        print(f"\n   [TARGET] TODO-PERF-001総合評価: [ERROR] 重大問題残存 ({remaining_count}件)")
