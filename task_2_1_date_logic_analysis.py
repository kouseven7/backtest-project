        ...
        if Path(engine_name).exists():
            ...
            try:
                ...
                if matches:
                    ...
                    print(f"  🎯 重要な処理: {len(important_lines)}件")
                    ...
                    if expected_year_lines:
                        print(f"  ⚠️ expected_year = 2023 の設定発見:")
                    ...
                    print(f"  ❌ _fix_date_inconsistencies メソッドが見つかりません")
                        ...
                        for date_col in date_columns:
                            if date_col in df.columns:
                                ...
                                if len(date_values) > 0:
                                    ...
                                    if loop_patterns:
                                        print(f"      ⚠️ 年末年始パターン: {len(loop_patterns)}件")
...
def detect_date_loop_patterns(results):
    ...
    try:
        ...
        if Path(backtester_path).exists():
            ...
            print(f"📊 ポートフォリオ関連メソッド: {len(portfolio_methods)}件")
            ...
            print(f"💰 ポートフォリオ値生成: {len(portfolio_value_lines)}件")
...
def main():
    ...
    print(f"✅ 分析完了エンジン: {engine_count}件")
    print(f"✅ Excel分析シート: {excel_sheets}件")
    print(f"✅ ループパターン検出: {'成功' if not analysis_results.get('date_loop_detection', {}).get('error') else '失敗'}")
    ...
    for engine, analysis in analysis_results.get('engine_date_fixes', {}).items():
        ...
        if expected_year_usage:
            print(f"  ⚠️ {engine}: expected_year=2023の固定設定発見")
