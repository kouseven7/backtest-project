...
try:
    ...
    def test_dssms_backtester_v2_excel():
        ...
        print("=" * 80)
        print("[TOOL] DSSMS Backtester V2 Excel出力統合テスト")
        print("=" * 80)
        
        try:
            # 1. DSSMS Backtester初期化
            print("\n[LIST] Step 1: DSSMS Backtester初期化")
            ...
            print("[OK] DSSMS Backtester初期化完了")
            
            # 2. サンプルバックテストデータ作成
            print("\n[LIST] Step 2: サンプルバックテストデータ作成")
            ...
            if output_path and Path(output_path).exists():
                print(f"[OK] V2 Excel出力成功: {output_path}")
...
import src.utils.openpyxl_lazy_wrapper as openpyxl
                    ...
                    if "サマリー" in sheet_names:
                        ...
                        print(f"[LIST] サマリーシート行数: {summary_sheet.max_row}")
                        print(f"[LIST] サマリーシート列数: {summary_sheet.max_column}")
                    
                    workbook.close()
                    print("[OK] Excelファイル内容確認完了")
                    
                except Exception as e:
                    print(f"[WARNING] Excelファイル内容確認エラー: {e}")
                
            else:
                print("[ERROR] V2 Excel出力失敗")
                return False
            
            # 4. パフォーマンス評価
            print("\n[LIST] Step 4: パフォーマンス評価")
            ...
            for i in range(3):
                test_output = backtester.export_results_to_excel(
                    simulation_result, performance_metrics, comparison_result)
                if test_output:
                    print(f"[OK] 出力テスト{i+1}: 成功")
                else:
                    print(f"[ERROR] 出力テスト{i+1}: 失敗")
            
            end_time = datetime.now()
            total_time = (end_time - start_time).total_seconds()
            
            print(f"⏱️ 3回出力の合計時間: {total_time:.2f}秒")
            print(f"[CHART] 平均出力時間: {total_time/3:.2f}秒/回")
            
            # 5. 最終レポート
            print("\n" + "=" * 80)
            print("[SUCCESS] DSSMS Backtester V2 Excel出力統合テスト完了")
            print("=" * 80)
            print("[OK] 全てのテストが成功しました")
            print(f"📁 最新出力ファイル: {output_path}")
            print("[ROCKET] 新しいV2システムがDSSMS Backtesterに正常に統合されました")
    ...
    if __name__ == "__main__":
        ...
        if success:
            print("\n[TARGET] テスト結果: DSSMS Backtester V2 Excel統合成功！")
    ...
    print("[SEARCH] DSSMSBacktesterクラスが見つかりません")
