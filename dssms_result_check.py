...
# ORIGINAL: def check_latest_excel_results():
    ...
    """
    logger.info("=" * 60)
    logger.info("[SEARCH] DSSMS修正結果の確認")
    logger.info("=" * 60)
    ...
    logger.info(f"[CHART] 最新結果ファイル: {latest_file.name}")
    ...
    try:
        ...
        if total_return > -50:
            logger.info(f"   [OK] -100%損失問題: 解決 ({total_return:.2f}%)")
        else:
            logger.error(f"   [ERROR] -100%損失問題: 未解決 ({total_return:.2f}%)")
        
        if total_trades > 0:
            logger.info(f"   [OK] 取引シグナル生成: 正常 ({int(total_trades)}件)")
        else:
            logger.error(f"   [ERROR] 取引シグナル生成: 失敗 (0件)")
        
        if profit_factor > 0:
            logger.info(f"   [OK] 利益係数: 正常 ({profit_factor:.2f})")
        else:
            logger.error(f"   [ERROR] 利益係数: 異常 ({profit_factor:.2f})")
        ...
        if total_return > 0:
            logger.info("   [OK] プラスリターン達成")
        elif total_return > -10:
            logger.warning("   [WARNING]  軽微な損失（許容範囲）")
        else:
            logger.error("   [ERROR] 大幅な損失")
        
        if win_rate > 50:
            logger.info(f"   [OK] 勝率良好 ({win_rate:.1f}%)")
        else:
            logger.warning(f"   [WARNING]  勝率改善余地 ({win_rate:.1f}%)")
        ...
        if total_return > buy_hold_return:
            ...
            logger.warning("   [DOWN] Buy & Holdを下回る")
        ...
        logger.error(f"[ERROR] Excel読み取りエラー: {e}")
        return None
    ...
    try:
        ...
        if not result.empty:
            ...
            if entry_signals > 0:
                ...
                logger.error("   [ERROR] Perfect Order修正失敗")
        
    except Exception as e:
        logger.error(f"[ERROR] シグナル分析エラー: {e}")

def main():
    """メイン実行"""
    logger.info("[ROCKET] DSSMS修正結果確認スクリプト開始")
...
# ORIGINAL: result = check_latest_excel_results()
    ...
    if result and result['problem_solved']:
        logger.info("[SUCCESS] DSSMS -100%問題の解決に成功しました！")
        logger.info("[OK] Perfect Order検出器の修正が効果的でした")
        logger.info("[UP] システムが正常に取引シグナルを生成しています")
        ...
        logger.warning("[WARNING]  部分的な改善は見られますが、まだ課題があります")
        logger.info(f"[IDEA] 現在のリターン: {result['total_return']:.2f}%")
        logger.info("[TOOL] さらなる調整が必要です")
        
    else:
        logger.error("[ERROR] 結果の確認に失敗しました")
