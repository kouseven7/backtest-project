...
try:
    ...
    print(f'[TARGET] VWAPBounceStrategy登録状況: {"[OK] 登録済み" if vwap_bounce_registered else "[ERROR] 未登録"}')
    ...
    print(f'[CHART] 戦略登録状況: {registered_count}/{len(expected_strategies)} ({registration_rate:.1f}%)')
    
    # 未登録戦略の確認
    unregistered = [s for s in expected_strategies if s not in available_strategies]
    if unregistered:
        print(f'[WARNING] 未登録戦略: {unregistered}')
    
    if vwap_bounce_registered:
        print('\\n[TEST] VWAPBounceStrategy動作確認テスト')
        ...
        try:
            ...
            print(f'[OK] VWAPBounceStrategy インスタンス化成功: backtest()={has_backtest}')
            ...
            if has_backtest:
                try:
                    ...
                    print(f'[OK] VWAPBounceStrategy backtest実行成功: {result.shape[0]}行')
                    
                    # シグナル列確認
                    required_columns = ['Entry_Signal', 'Exit_Signal']
                    missing_columns = [col for col in required_columns if col not in result.columns]
                    if not missing_columns:
                        print('[OK] VWAPBounceStrategy: Entry_Signal/Exit_Signal列確認済み')
                    else:
                        print(f'[WARNING] VWAPBounceStrategy: シグナル列不足 {missing_columns}')
                        
                except Exception as e:
                    print(f'[WARNING] VWAPBounceStrategy backtest実行エラー: {e}')
                    
        except Exception as e:
            print(f'[ERROR] VWAPBounceStrategy インスタンス化エラー: {e}')
    
    # 戦略レジストリステータス
    registry_status = manager.get_registry_status()
    print(f'\\n[LIST] レジストリステータス: 初期化={registry_status["is_initialized"]}, 戦略数={registry_status["total_strategies"]}')
    
    print(f'\\n[TARGET] 検証結果サマリー:')
    print(f'  VWAPBounceStrategy登録: {"[OK]" if vwap_bounce_registered else "[ERROR]"}')
    print(f'  7/7戦略完全登録: {"[OK]" if registration_rate == 100.0 else "[ERROR]"}')
    print(f'  戦略レジストリ品質: {registration_rate:.1f}%')
    
    if registration_rate == 100.0:
        print('[SUCCESS] VWAPBounceStrategy修正成功！7/7戦略完全登録達成！')
    else:
        print('[WARNING] まだ修正が必要な戦略があります')
